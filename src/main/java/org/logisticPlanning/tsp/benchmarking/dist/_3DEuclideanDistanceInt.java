 ...
 * </p>
 * <p>
 * _3DEuclideanDistanceInt represents list of 3-dimensional 32 bit signed
 * integer (int) coordinates between which the Euclidean measure is
 ...
 * formula</a>. In the 3-dimensional TSPs given in&nbsp;[<a
 * href="#cite_R1995T9" style="font-weight:bold">1</a>], this is computed
 * as
 * <code>nint(sqrt( [x1-x2]<sup>2</sup> + [y1-y2]<sup>2</sup> + [z1-z2]<sup>2</sup>))</code>
 ...
 * </ol>
 */
final class _3DEuclideanDistanceInt extends _3DIntCoords {
   ...
   *          the dimension
   */
  _3DEuclideanDistanceInt(final int n) {
    super(n);
  }
  ...
  public final int distance(final int i, final int j) {
    ...
    return ((int) (0.5d + (Math.sqrt((x * x) + (y * y) + (z * z)))));
  }
  ...
  public final long evaluate(final int[] nodes) {
    ...
    long totalDist;
    final int[] m;
    int x1, x2, xd, y1, y2, yd, z1, z2, zd;
  ...
  public final void print(final PrintWriter out) {
    super.print(out);
    this._print(out, _Constants.EUCLIDEAN_3D_STR);
  }
}
