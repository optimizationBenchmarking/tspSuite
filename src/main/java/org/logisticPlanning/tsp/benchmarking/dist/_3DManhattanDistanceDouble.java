 ...
 * </p>
 * <p>
 * _3DManhattanDistanceDouble represents list of 3-dimensional
 * double-precision (64-bit) floating point number (double) coordinates
 * between which the Manhattan measure is computed as specified in&nbsp;[<a
 ...
 * </ol>
 */
final class _3DManhattanDistanceDouble extends _3DDoubleCoords {
   ...
   *          the dimension
   */
  _3DManhattanDistanceDouble(final int n) {
    super(n);
  }
  ...
  public final int distance(final int i, final int j) {
    ...
    a = ((j - 1) * 3);
    return ((int) (0.5d + (Math.abs(x - m[a++]) + Math.abs(y - m[a++]) + Math
        .abs(z - m[a]))));
  }
  ...
  public final long evaluate(final int[] nodes) {
    ...
    for (final int cur_i : nodes) {
      ...
      totalDist += ((int) (0.5d + (Math.abs(x2 - x1) + Math.abs(y2 - y1) + Math
          .abs(z2 - z1))));
      x2 = x1;
      y2 = y1;
      z2 = z1;
