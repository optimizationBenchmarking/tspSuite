 ...
 * </p>
 * <p>
 * _3DMaximumDistanceInt represents list of 3-dimensional 32 bit signed
 * integer (int) coordinates between which the Maximum measure is computed
 * as specified in&nbsp;[<a href="#cite_R1995T9"
 * style="font-weight:bold">1</a>].
 * </p>
 * <p>
 * The Maximum distance (or <a
 * href="https://en.wikipedia.org/wiki/Chebyshev_distance">Chebyshev
 * distance</a>) between two points is the greatest of their differences
 * along any coordinate dimension&nbsp;[<a href="#cite_APR2002HOMDS"
 * style="font-weight:bold">17</a>, <a href="#cite_C2000MMMFPAE"
 * style="font-weight:bold">18</a>]. In the 3-dimensional TSPs given
 * in&nbsp;[<a href="#cite_R1995T9" style="font-weight:bold">1</a>], this
 * is computed as
 * <code>max{ nint(|x1-x2|),  nint(|y1-y2|), nint(|z1-z2|)}</code>, where
 * <code>nint</code> rounds to the nearest integer value.
 * </p>
 * <p>
 ...
 * </ol>
 */
final class _3DMaximumDistanceInt extends _3DIntCoords {
   ...
   *          the dimension
   */
  _3DMaximumDistanceInt(final int n) {
    super(n);
  }
  ...
  public final int distance(final int i, final int j) {
    ...
    return (Math.max(Math.abs(x - m[a++]),
        Math.max(Math.abs(y - m[a++]), Math.abs(z - m[a++]))));
  }
  ...
  public final long evaluate(final int[] nodes) {
    ...
    for (final int cur_i : nodes) {
      ...
      totalDist += (Math.max(Math.abs(x1 - x2),
          Math.max(Math.abs(y1 - y2), Math.abs(z1 - z2))));
  ...
  public final void print(final PrintWriter out) {
    super.print(out);
    this._print(out, _Constants.MAXIMUM_3D_STR);
  }
}
