 ...
 * </p>
 * <p>
 * _2DCeiledEuclideanDistanceInt represents list of 2-dimensional 32 bit
 * signed integer (int) coordinates between which the CeiledEuclidean
 * measure is computed as specified in&nbsp;[<a href="#cite_R1995T9"
 ...
 * formula</a>. The ceiling of this distance is, well, the result of the
 * Euclidean distance <emph>rounded up</emph> to the next integer. In the
 * 2-dimensional TSPs given in&nbsp;[<a href="#cite_R1995T9"
 * style="font-weight:bold">1</a>], this is computed as
 * <code>ceil(sqrt( [x1-x2]<sup>2</sup> + [y1-y2]<sup>2</sup></code>, where
 * <code>ceil</code> rounds up to the next integer value.
 * </p>
 * <p>
 ...
 * </ol>
 */
final class _2DCeiledEuclideanDistanceInt extends _2DIntCoords {
   ...
   *          the dimension
   */
  _2DCeiledEuclideanDistanceInt(final int n) {
    super(n);
  }
  ...
  public final int distance(final int i, final int j) {
    ...
    return ((int) (0.5d + Math.ceil(Math.sqrt((x * x) + (y * y)))));
  }
  ...
  public final long evaluate(final int[] nodes) {
    ...
    for (final int cur_i : nodes) {
      ...
      totalDist += ((int) (0.5d + Math.ceil(Math.sqrt((xd * xd)
          + (yd * yd)))));
      x2 = x1;
      y2 = y1;
  ...
  public final void print(final PrintWriter out) {
    super.print(out);
    this._print(out, _Constants.EUCLIDEAN_CEIL_2D_STR);
  }
}
