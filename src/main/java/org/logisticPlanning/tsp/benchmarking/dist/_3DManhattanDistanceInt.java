 ...
 * </p>
 * <p>
 * _3DManhattanDistanceInt represents list of 3-dimensional 32 bit signed
 * integer (int) coordinates between which the Manhattan measure is
 * computed as specified in&nbsp;[<a href="#cite_R1995T9"
 * style="font-weight:bold">1</a>].
 * </p>
 * <p>
 * The <a href="https://en.wikipedia.org/wiki/Taxicab_geometry">Manhattan
 * distance</a> between two points is the sum of the absolute differences
 * of their coordinates&nbsp;[<a href="#cite_K1986TGAAINEG"
 * style="font-weight:bold">2</a>]. In the 3-dimensional TSPs given
 * in&nbsp;[<a href="#cite_R1995T9" style="font-weight:bold">1</a>], this
 * is computed as <code>nint(|x1-x2| + |y1-y2| + |z1-z2|)</code>, where
 ...
 * </ol>
 */
final class _3DManhattanDistanceInt extends _3DIntCoords {
   ...
   *          the dimension
   */
  _3DManhattanDistanceInt(final int n) {
    super(n);
  }
  ...
  public final int distance(final int i, final int j) {
    ...
    a = ((j - 1) * 3);
    return (Math.abs(x - m[a++]) + Math.abs(y - m[a++]) + Math.abs(z
        - m[a]));
  }
  ...
  public final long evaluate(final int[] nodes) {
    ...
    for (final int cur_i : nodes) {
      ...
      totalDist += (Math.abs(x2 - x1) + Math.abs(y2 - y1) + Math.abs(z2
          - z1));
      x2 = x1;
      y2 = y1;
      z2 = z1;
  ...
  public final void print(final PrintWriter out) {
    super.print(out);
    this._print(out, _Constants.MANHATTAN_3D_STR);
  }
}
