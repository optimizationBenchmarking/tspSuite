 ...
 * </p>
 * <p>
 * _2DMaximumDistanceInt represents list of 2-dimensional 32 bit signed
 ...
 * style="font-weight:bold">18</a>]. In the 2-dimensional TSPs given
 * in&nbsp;[<a href="#cite_R1995T9" style="font-weight:bold">1</a>], this
 * is computed as <code>max{ nint(|x1-x2|),  nint(|y1-y2|)}</code>, where
 ...
 * </ol>
 */
final class _2DMaximumDistanceInt extends _2DIntCoords {
   ...
   *          the dimension
   */
  _2DMaximumDistanceInt(final int n) {
    super(n);
  }
  ...
  public final int distance(final int i, final int j) {
    ...
    return (Math.max(Math.abs(x - m[a++]), Math.abs(y - m[a++])));
  }
  ...
  public final long evaluate(final int[] nodes) {
    ...
    for (final int cur_i : nodes) {
      ...
      totalDist += (Math.max(Math.abs(x1 - x2), Math.abs(y1 - y2)));
  ...
  public final void print(final PrintWriter out) {
    super.print(out);
    this._print(out, _Constants.MAXIMUM_2D_STR);
  }
}
