 ...
 * </p>
 * <p>
 * _3DDoubleCoords represents a list of 3-dimensional double-precision
 * (64-bit) floating point number (double) coordinates. This class is
 * internally extended with different distance measures, see
 * {@link _3DManhattanDistanceDouble}, {@link _3DEuclideanDistanceDouble},
 * {@link _3DMaximumDistanceDouble}, {@link _3DXRay1DistanceDouble}.
 * </p>
 * <p>
 * The layout of this list is such all 3 {@code double} values belonging to
 ...
 * you should first load all 3 coordinates of the first point onto the
 ...
 */
abstract class _3DDoubleCoords extends DistanceComputer {

  /** the coordinates */
  transient double[] m_coords;
   ...
   *          the dimension
   */
  _3DDoubleCoords(final int n) {
    super(n);
    this.m_coords = new double[((n) * 3)];
  }
   ...
   *          the z-coordinate
   */
  final void setCoords(final int i, final double x, final double y,
      final double z) {
    int a;
    double[] c;
  ...
  final void load(final BufferedReader in) throws IOException {
    ...
    while ((s = in.readLine()) != null) {
      ...
      if (_Constants.NODE_COORD_SECTION_STR.equalsIgnoreCase(s)) {
        ...
        for (i = this.m_n; (--i) >= 0;) {
          this.setCoords(nums.nextInt(), nums.nextDouble(),
              nums.nextDouble(), nums.nextDouble());
        }
        return;
      }
