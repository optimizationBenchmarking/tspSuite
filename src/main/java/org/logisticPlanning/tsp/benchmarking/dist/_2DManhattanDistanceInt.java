 ...
 * </p>
 * <p>
 * _2DManhattanDistanceInt represents list of 2-dimensional 32 bit signed
 ...
 * style="font-weight:bold">2</a>]. In the 2-dimensional TSPs given
 * in&nbsp;[<a href="#cite_R1995T9" style="font-weight:bold">1</a>], this
 * is computed as <code>nint(|x1-x2| + |y1-y2|</code>, where
 ...
 * </ol>
 */
final class _2DManhattanDistanceInt extends _2DIntCoords {
   ...
   *          the dimension
   */
  _2DManhattanDistanceInt(final int n) {
    super(n);
  }
  ...
  public final int distance(final int i, final int j) {
    ...
    a = ((j - 1) << 1);
    return (Math.abs(x - m[a++]) + Math.abs(y - m[a]));
  }
  ...
  public final long evaluate(final int[] nodes) {
    ...
    for (final int cur_i : nodes) {
      ...
      totalDist += (Math.abs(x2 - x1) + Math.abs(y2 - y1));
      x2 = x1;
      y2 = y1;
  ...
  public final void print(final PrintWriter out) {
    super.print(out);
    this._print(out, _Constants.MANHATTAN_2D_STR);
  }
}
