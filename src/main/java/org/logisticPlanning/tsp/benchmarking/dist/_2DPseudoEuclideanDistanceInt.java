 ...
 * </p>
 * <p>
 * _2DPseudoEuclideanDistanceInt represents list of 2-dimensional 32 bit
 * signed integer (int) coordinates between which the PseudoEuclidean
 * measure is computed as specified in&nbsp;[<a href="#cite_R1995T9"
 * style="font-weight:bold">1</a>].
 * </p>
 * <p>
 * The pseudo-Euclidean distance is a rounded form of the Euclidean
 * distance. The <a
 ...
 * xd = x1 - x2<br/>
 * yd = y1 - y2<br/>
 * r  = sqrt( (x2<sup>2</sup> + y2<sup>2</sup>) / 10.0 )<br/>
 * t  = nint(r)<br/>
 * if (t < r) return (t + 1)<br/>
 * return t;
 * </pre>
 * <p>
 ...
 * </ol>
 */
final class _2DPseudoEuclideanDistanceInt extends _2DIntCoords {
   ...
   *          the dimension
   */
  _2DPseudoEuclideanDistanceInt(final int n) {
    super(n);
  }
  ...
  public final long evaluate(final int[] nodes) {
    ...
    for (final int cur_i : nodes) {
      ...
      rij = Math.sqrt(((xd * xd) + (yd * yd)) * 0.1d);
      tij = ((int) (0.5d + (rij)));

      totalDist += ((tij < rij) ? (tij + 1) : tij);
      x2 = x1;
      y2 = y1;
  ...
  public final void print(final PrintWriter out) {
    super.print(out);
    this._print(out, _Constants.EUCLIDEAN_PSEUDO_2D_STR);
  }
}
