 ...
 * for finding optimal path sub-sequences of length 3.
 * </p>
 * <p>
...
public final class ExhaustivelyEnumeratingLocal3Optimizer
   ...
   * {@link #m_best_node_3}; otherwise these variables are not used
   */
   ...
   * {@link ExhaustivelyEnumeratingLocal3Optimizer}
   */
  public ExhaustivelyEnumeratingLocal3Optimizer() {
    super(3);
  }

  /**
   * This is the main entry point of class
   * {@link ExhaustivelyEnumeratingLocal3Optimizer}. This method makes a
   * sub-sequence of length 3 inside {@code path}, starting right after
  ...
  public final long apply(final int[] path, final int beforeStart,
    ...
    // Now processing permutation (1,3,2).
    current_delta = ((this.m_dist_1_3 + this.m_dist_2_afterEnd)
        - this.m_dist_1_2 - this.m_dist_3_afterEnd);
    if (current_delta < this.m_best_delta) {
      // A new best permutation has been discovered: (1,3,2)
   ...
   * from (1,3,2). &quot;Efficiently&quot; means that there is no shorter
   ...
   * 
   * @param parent_delta
   *          the length change between the original tour and (1,3,2)
   */
  private final void __trace_1(final long parent_delta) {

    long current_delta;

    // Now processing permutation (2,3,1).
    ...
    if (current_delta < this.m_best_delta) {
      // A new best permutation has been discovered: (2,3,1)
      this.m_best_delta = current_delta;
      this.m_best_node_1 = this.m_original_node_2;
      this.m_best_node_2 = this.m_original_node_3;
      this.m_best_node_3 = this.m_original_node_1;
    }
    // Finished processing permutation (2,3,1).

    // Now processing permutation (3,1,2).
    current_delta = ((parent_delta + this.m_dist_beforeStart_3 + this.m_dist_1_2)
        - this.m_dist_beforeStart_1 - this.m_dist_2_3);
    if (current_delta < this.m_best_delta) {
      // A new best permutation has been discovered: (3,1,2)
      this.m_best_delta = current_delta;
      this.m_best_node_1 = this.m_original_node_3;
      this.m_best_node_2 = this.m_original_node_1;
      this.m_best_node_3 = this.m_original_node_2;
    }
    // Finished processing permutation (3,1,2).
  }

  /** {@inheritDoc} */
  @Override
  public final ExhaustivelyEnumeratingLocal3Optimizer clone() {
    return ((ExhaustivelyEnumeratingLocal3Optimizer) (super.clone()));
  }
