...
/**
 * An update operation can compute the change in objective value of a candidate
 * solution by only few invocations of the distance function and then be applied
 * requiring only few modifications to a candidate solution.
 */
public class PermutationUpdateOperator extends UnaryOperator<int[]> {
	...
	/**
	 * a constant returned by {@link #delta(int[], DistanceComputer, int, int)}
	 * if the application (
	...
	/**
	 * Compute the distance after applying the operation. If this method returns
	 * the constant value {@link #NO_EFFECT}, this means that applying the
	 * operator via {@link #apply(Individual, ObjectiveFunction, Individual...)}
	 * would have no effect, i.e., leave the solution unchanged. Please be very
	...
	/**
	 * Return the edges added by this operator, need to be implemented by the subclasses
	 * @param <E>
	 *
	 * @param perm
	 *            the permutation to be changed
	 ...
	 *            the second index parameter
	 */
	public ArrayList<_PairOfNode> inComingEdges(final int[] perm, final int a,
			final int b) {
		return null;
	}

	/**
	 * Return the edges deleted by this operator, need to be implemented by the subclasses
	 *
	 * @param perm
	 *            the permutation to be changed
	 * @param a
	 ...
	 *            the second index parameter
	 */
	public ArrayList<_PairOfNode> outComingEdges(final int[] perm, final int a,
			final int b) {
		return null;
	}
