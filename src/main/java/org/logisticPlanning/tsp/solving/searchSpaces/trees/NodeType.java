...
public class NodeType<NT extends Node<CT>, CT extends Node<CT>> extends
  ...
  private final NodeTypeSet<CT>[] m_chTypes;
  ...
  public NodeType(final String name, final NodeTypeSet<CT>[] ch) {
    super(name);

    this.m_chTypes = (((ch != null) && ((ch.length) > 0)) ? ((NodeTypeSet[]) ch)
        : (NodeType.EMPTY_CHILDREN));
  }
  ...
  public final boolean isTerminal() {
    return (this.m_chTypes.length <= 0);
  }
