...
public class SatelliteNode {
  ...
  /** the first pointer to another node in the sequence */
  SatelliteNode m_p1;

  /** the second pointer to another node in the sequence */
  SatelliteNode m_p2;

  /**
   * A tag used to check if the node has already been visited, i.e., to
   ...
   *          the node id
   */
  protected SatelliteNode(final int pid) {
    super();
    this.id = pid;
  }

  /**
   * Get the neighbor node at the given index. A satellite node can have up
   * to two neighboring nodes. Their indexes are {@code 0} and {@code 1}.
   * If a neighbor is not connected, the method will return {@code null},
   ...
   */
  public SatelliteNode getNeighbor(final int index) {
    if (index <= 0) {
      return this.m_p1;
    }
    return this.m_p2;
  ...
  public boolean isBalanced() {
    return ((this.m_p1 != null) && (this.m_p2 != null));
  }

  /**
   * Commit any pending changes. This method does nothing in this class,
   * but is overridden in class
   * {@link org.logisticPlanning.tsp.solving.utils.satelliteList.UndoableSatelliteNode}
   * to do useful stuff.
   */
  void commit() {
   ...
   */
  void doDisconnect(final SatelliteNode node) {
   ...
   */
  void undoDisconnect(final SatelliteNode node) {
    if (this.m_p1 == null) {
      this.m_p1 = node;
    } else {
      this.m_p2 = node;
   ...
   */
  void doConnect(final SatelliteNode node) {
    if (this.m_p1 == null) {
      this.m_p1 = node;
    } else {
      this.m_p2 = node;
   ...
   */
  void undoConnect(final SatelliteNode node) {
    if (node == this.m_p1) {// if node n is the m_p1 node, we delete it
      this.m_p1 = null;
    } else {// if it is not m_p1, it must be m_p2
      this.m_p2 = null;
   ...
   */
  public final boolean isNeighbor(final SatelliteNode node) {
