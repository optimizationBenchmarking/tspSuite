 ...
 *         >China</a> [&#x4E2D;&#x56FD;]
 */
public class TabuSearchEx extends TSPLocalSearchAlgorithm<int[]> {
	...
	public final static void main(final String[] args) {
		TSPAlgorithmRunner.benchmark(//
				Instance.SYMMETRIC_INSTANCES , TabuSearchEx.class,//
				args);
	}
	...
	/** {@inheritDoc} */
	@Override
	public final TabuSearchEx clone() {
		final TabuSearchEx res;
		final PermutationUpdateOperator[] ops;
		int i;

		res = ((TabuSearchEx) (super.clone()));
	...
	public void configure(final Configuration config) {
		...
		this.absolute = config.getInt(TabuSearchEx.PARAM_ABSOLUTE_LENGTH, 0,
				20000, this.absolute);
		this.rate = config.getInt(TabuSearchEx.PARAM_RATE, 0, 1000, this.rate);
	}
	...
	public void printConfiguration(final PrintStream ps) {
		...
		Configurable.printKey(TabuSearchEx.PARAM_ABSOLUTE_LENGTH, ps);
		Configurable.printlnObject(this.tabuListLength, ps);
		Configurable.printKey(TabuSearchEx.PARAM_RATE, ps);
		Configurable.printlnObject(this.rate, ps);
		Configurable.printKey(TabuSearchEx.PARAM_UPDATING_OPERATORS, ps);
