...
/**
 * In the improved P_Sec, we change the Rego's tabu rules "one edge that is deleted can not be added"
 * to the rule "one edge that is deleted can not be deleted again" in the algorithm, and reduce 
 * the available root nodes from n to 0.15*n, the number of level is from the maximum 2*n to 0.45*n.
 * In that way, we get the much more improvement performance.
 */
public class P_Sec extends TSPLocalSearchAlgorithm<int[]> {
	...
	/** create */
	public P_Sec() {
		super("P_Sec Algorithm using a Stem-and-Cycle Datastructure");
	}
	/**
	 * The main routine
	 * 
	 ...
	 * @throws Exception to find where is the error
	 */
