...
public class TabuSearchMove extends TSPLocalSearchAlgorithm<int[]> {
  ...
  public void localSearch(final Individual<int[]> srcdst,
    ...
    _PairOfNode pairOfEx = new _PairOfNode();
    final int n;
    ...
    while (foundImprov && (!(f.shouldTerminate()))) {
      ...
      if (best != null) {
        pairOfEx = new _PairOfNode(sol[bestA], sol[bestB]);
        best.update(sol, bestA, bestB);
        if (!this.tabuList.isEmpty()
            && (this.tabuList.size() >= this.tabuListLength)) {
  ...
  public void beginRun(final ObjectiveFunction f) {
    ...
    this.tabuList = new ArrayList<>(this.tabuListLength);
  }
