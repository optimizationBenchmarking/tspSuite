 ...
 *         Technology of China (USTC)</a> [&#x4E2D;&#x56FD;&#x79D1;&#x5B66
 *         ;&#x6280;&#x672F;&#x5927;&#x5B66;], (<a href=
...
public class TabuSearchSolution extends TSPLocalSearchAlgorithm<int[]> {
	...
	public void localSearch(final Individual<int[]> srcdst,
		...
		PathHash useHash;
		int currentDelta, currentStart, currentEnd, oldHash, bestDelta, bestStart, bestEnd;
		long performedFEs;
		...
		while (foundImprov &&(!(f.shouldTerminate()))) {
			...
			findNewSolution: {// find a new solution
				...
				for (currentEnd = n; (--currentEnd) > 0;) {
					for (currentStart = currentEnd; (--currentStart) >= 0;) {
						...
						for (final PermutationUpdateOperator currentOperator : this.m_ops) {
							...
							// operation?
							currentDelta = currentOperator.delta(
									solutionHash.m_data, f, currentStart,
									currentEnd);

							// is the new solution better than the best
							// permissible one
							// we have seen?
							if (currentDelta < bestDelta) { // only then we need
															// to
								// apply the operation and get the hash code
								currentOperator.update(solutionHash.m_data,
										currentStart, currentEnd);
								performedFEs++;
								oldHash = solutionHash.m_hash;
								// this is a very costly operation, so avoid
								// where possible
								...
								if (!(this.m_tabuList.contains(solutionHash))) {
									...
									if (bestDelta < 0) {// at first improvement:
										...
										if (this.m_useFirstImprovement) {
											break findNewSolution; 
											// don't revert, keep solution
										}
									}
								}

								// undo operation: we may find something better
								// later on
								currentOperator.revertUpdate(
										solutionHash.m_data, currentStart,
										currentEnd);
				...
				// OK, the main loop has terminated, every possible op(start,
				// end) has been tested.
				...
				if(bestDelta < 0)foundImprov = true;
				...
				if (this.m_tabuListLength > 0) {
					if (this.m_tabuList.size() >= this.m_tabuListLength) {
						...
						if (iteratorToGetFirst.hasNext()) {// should be true
							useHash = iteratorToGetFirst.next(); // get first
																	// tabu
							iteratorToGetFirst.remove(); // delete it

							// reuse old hash object: avoid unnecessary
							// allocation
							System.arraycopy(solutionHash.m_data, 0,
									useHash.m_data, 0, n);
