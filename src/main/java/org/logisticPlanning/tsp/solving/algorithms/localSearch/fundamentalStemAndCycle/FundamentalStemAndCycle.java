...
/**
 * The fundamental stem and cycle algorithm working internally on a
 * stem-and-cycle data structure.
 */
public class FundamentalStemAndCycle extends
TSPLocalSearchAlgorithm<int[]> {
  ...
  public FundamentalStemAndCycle() {
    super(
        "Subpath Ejection Algorithm using a fundamental Stem-and-Cycle Datastructure"); //$NON-NLS-1$
  }
  ...
  public void localSearch(final Individual<int[]> srcdst,
    ...
    // this.__printTabuEdge();
    // this.__printTabuList();
    // System.exit(1);
  }
  ...
  private final int __getRandomRoot() {
    ...
    if (this.m_rootListLength != 0) {
      ...
      System.out.println("root list is empty!!"); //$NON-NLS-1$
      final int[] i = new int[2];
      i[2] = 8;
      System.exit(1);
    }
    if (this.debug) {
      System.out.println("root" + root);//$NON-NLS-1$
    }
    return root;
  }
  ...
  private final void __secRootNoChanged() {
    ...
    while (countLevel < this.m_level) {
      ...
      if ((jOnCycle != null)
        ...
        // System.out.println("j on cycle Delete edge " + jOnCycle[2] + " "
        // + jOnCycle[3]);
  ...
  private final long __getBetterTour() {
    ...
    if (this.debug) {
      System.out.println("Tour Length" + tourLength); //$NON-NLS-1$
    }
    return tourLength;
  }
