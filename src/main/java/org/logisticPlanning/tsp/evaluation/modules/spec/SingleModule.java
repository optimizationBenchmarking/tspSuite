...
/**
 * A module that evaluates a single experiment.
 */
public class SingleModule extends _ConcreteModule {
   ...
   */
  protected SingleModule(final String name, final Module owner,
   ...
   * @return the label
   */
  public final Label getLabel(final Experiment data) {
  ...
  final void _writeSectionTitle(final SectionTitle title,
    ...
    if (data instanceof Experiment) {
      this.writeSectionTitle(title, ((Experiment) data));
    } else {
      this._logModuleCanOnlyProcessWarning(Experiment.class, data,
  ...
  final void _writeSectionBody(final SectionBody body,
      final DataSet<?> data) throws IOException {
    if (data instanceof Experiment) {
      this.writeSectionBody(body, ((Experiment) data));
    } else {
      this._logModuleCanOnlyProcessWarning(Experiment.class, data,
  ...
  final void _initialize(final Header header, final DataSet<?> data)
      throws IOException {//
    if (data instanceof Experiment) {
      this.initialize(header, ((Experiment) data));
    } else {
      this._logModuleCanOnlyProcessWarning(Experiment.class, data,
  ...
  final boolean hasContent(final DataSet<?> data) {
    if (data instanceof Experiment) {
      return this.hasContent((Experiment) data);
    }
    this._logModuleCanOnlyProcessWarning(Experiment.class, data,
