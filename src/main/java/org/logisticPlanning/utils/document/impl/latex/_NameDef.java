...
/**
 * <p>
 * The internal for defining math names.
 * </p>
 * <p>
 ...
 * </ol>
 */
final class _NameDef {

  /** the map with the operator data */
  static final EnumMap<EMathName, _NameDef> MAP;

  static {
    final EnumMap<EMathName, _NameDef> map;
    final char[] ch;

    ch = new char[] { '{', '\\', 't', 'e', 'x', 't', 'i', 't', '{' };

    map = new EnumMap<>(EMathName.class);

    map.put(EMathName.SCALAR,//
        new _NameDef(ch, 2));

    map.put(EMathName.VECTOR,//
        new _NameDef(new char[] { '{', '\\', 'v', 'e', 'c', '{' }, 2));

    map.put(EMathName.MATRIX,//
        new _NameDef(new char[] { '{', '\\', 't', 'e', 'x', 't', 'b', 'f',
            '{', '\\', 't', 'e', 'x', 't', 'i', 't', '{' }, 3));

    map.put(EMathName.SET,//
        new _NameDef(ch, 2));
   ...
   *          the end string
   */
  private _NameDef(final char[] start, final int end) {
    super();
    this.m_start = start;
    this.m_end = end;
  }
