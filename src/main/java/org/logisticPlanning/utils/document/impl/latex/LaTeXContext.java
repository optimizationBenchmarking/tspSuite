...
/**
 * <p>
 * A context for <a href="http://en.wikipedia.org/wiki/LaTeX">LaTeX</a>
 ...
 * </ol>
 */
public class LaTeXContext extends Context {

  /** the LaTeX document file */
   ...
   *           if io fails
   */
  protected LaTeXContext(final LaTeXDriver driver, final File baseDir)
      throws IOException {
  ...
  /** {@inheritDoc} */
  @Override
  public LaTeXDriver getDriver() {
    return ((LaTeXDriver) (super.getDriver()));
  }
   ...
   *           if io fails
   */
  protected LaTeXDocument documentInstantiate(final LaTeXContext owner,
      final File f) throws IOException {
    return new LaTeXDocument(owner, f);
  }

  /** {@inheritDoc} */
  @Override
  protected final LaTeXDocument documentCreate(final Context owner)
      throws IOException {
    final Logger log;
   ...
   *           if io fails
   */
  protected String createBibliography(final BibReference[] refs)
      throws IOException {
    final File fr;
    final String name, fullName;
    final Logger log;

    name = "bibliography"; //$NON-NLS-1$
    fullName = name + ".bib"; //$NON-NLS-1$

    fr = FileUtils.canonicalize(new File(this.getBaseDir(), fullName));
    ...
    try (FileOutputStream fos = new FileOutputStream(fr)) {
      try (OutputStreamWriter osw = new OutputStreamWriter(fos,
        ...
        try (BufferedWriter bw = new BufferedWriter(osw)) {
          _Bibliography._writeReferences(refs, new WriterCharOutput(bw));
        }
      }
    }
  ...
  protected void doPostprocess() throws IOException {
    ...
    log = this.getLogger();
    if ((log != null) && (log.isLoggable(Level.INFO))) {
      log.info("Begin of post-processing of LaTeX output."); //$NON-NLS-1$
    }
    ...
    try {
      ...
      synchronized (driver) {
        if (driver.shouldCompileLaTeX()) {
          c = new _Compiler(this);
        } else {
          c = null;
        }
      }
      ...
      if ((log != null) && (log.isLoggable(Level.INFO))) {
        log.info((c != null) ? "Now invoking compiler." : //$NON-NLS-1$
            "Compilation is turned off, so we are finished here."); //$NON-NLS-1$
      }
